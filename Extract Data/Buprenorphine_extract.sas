...
proc sql;
/*insert into Rx_ext.Buprenorphine_products
select *
from Rxdata.Products
where upper(MKTED_PROD_NM) like upper('%Buprenorphine%') OR
	upper(MKTED_PROD_NM) like upper('%BUTRANS%') OR
	upper(MKTED_PROD_NM) like upper('%BELBUCA%'); */

create table Rx_ext.Buprenorphine_overall
...
insert into Rx_ext.Buprenorphine_overall
...
where PRODUCT_ID in (
	select PRODUCT_ID
	from Rx_ext.Buprenorphine_products);
